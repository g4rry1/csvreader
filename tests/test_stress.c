...
static FILE *open_stream(const char *s) { return fmemopen((void *)s, strlen(s), "r"); }
...
static void test_stress_valid_cell_ref_formulas(void **state) {
    ...
    for (int iter = 0; iter < 150; iter++) {
        ...
        for (int r = 0; r < n_rows; r++) {
            sb_appendf(&sb, "%d", row_nums[r]);
            for (int c = 0; c < n_cols; c++) {
                if (use_ref[r][c]) { /* "=<prev_col_letter><row_num>+<literal>" */
    ...
    for (int iter = 0; iter < 100; iter++) {
        ...
        for (int r = 0; r < n_rows; r++) {
            ...
            for (int c = 0; c < n_cols; c++) {
                if (r == bad_r && c == bad_c) {
                    if (rand() % 2 == 0) {
                        sb_appendf(&sb, ",=Z%d+1", row_nums[r]); /* column Z doesn't exist */
                    } else {
                        sb_appendf(&sb, ",=%c999+1", 'A' + bad_c); /* row 999 doesn't exist */
                    }
                } else {
                    sb_appendf(&sb, ",%d", rng(1, 100));
                }
...
static void test_stress_invalid_self_ref(void **state) {
    ...
    for (int iter = 0; iter < 100; iter++) {
        ...
        for (int r = 0; r < n_rows; r++) {
            ...
            for (int c = 0; c < n_cols; c++) {
                if (r == bad_r && c == bad_c) {
                    sb_appendf(&sb, ",=%c%d+1", 'A' + bad_c, row_nums[bad_r]); /* self-reference */
                } else {
                    sb_appendf(&sb, ",%d", rng(1, 100));
                }
            }
...
static void test_stress_invalid_mutual_circular(void **state) {
    ...
    for (int iter = 0; iter < 100; iter++) {
        ...
        for (int r = 0; r < n_rows; r++) {
            sb_appendf(&sb, "%d", row_nums[r]);
            for (int c = 0; c < n_cols; c++) {
                if (r == cycle_r && c == cycle_c1) { /* left cell references right cell */
